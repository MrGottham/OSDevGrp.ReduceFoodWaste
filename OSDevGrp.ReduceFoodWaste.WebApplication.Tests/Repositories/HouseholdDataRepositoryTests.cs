...
namespace OSDevGrp.ReduceFoodWaste.WebApplication.Tests.Repositories
{
    ...
    public class HouseholdDataRepositoryTests : TestBase
    {
        ...
        /// <summary>
        /// Tests that GetPaymentHandlersAsync throws an ArgumentNullException when the identity is null.
        /// </summary>
        [Test]
        public void TestThatGetPaymentHandlersAsyncThrowsArgumentNullExceptionWhenIdentityIsNull()
        {
            var householdDataRepository = CreateHouseholdDataRepository();
            Assert.IsNotNull(householdDataRepository);

            var exception = Assert.Throws<ArgumentNullException>(() => householdDataRepository.GetPaymentHandlersAsync(null, Thread.CurrentThread.CurrentUICulture));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Empty);
            Assert.That(exception.ParamName, Is.EqualTo("identity"));
        ...
        /// <summary>
        /// Tests that GetPaymentHandlersAsync throws an ArgumentNullException when the culture informations which should be used for translation is null.
        /// </summary>
        [Test]
        public void TestThatGetPaymentHandlersAsyncThrowsArgumentNullExceptionWhenCultureInfoIsNull()
        {
            var householdDataRepository = CreateHouseholdDataRepository();
            Assert.IsNotNull(householdDataRepository);

            var exception = Assert.Throws<ArgumentNullException>(() => householdDataRepository.GetPaymentHandlersAsync(MockRepository.GenerateMock<IIdentity>(), null));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Empty);
            Assert.That(exception.ParamName, Is.EqualTo("cultureInfo"));
